...
export default function Home() {
  ...
  useEffect(() => {
    setList(searchData);
    setfacilitiesList([
      ...new Set(searchData.map((e) => e.hotel.content.hotelFacilities).flat())
    ]);
  }, [searchData]);
    ...
    (data) => {
      ...
      const filteredByStarRating = filteredByFacility.filter((li) => {
        return (
          ...
          +li.hotel.content.starRating >= data.star
        );
      });
          ...
          <div className={classes['main-container']}>
            {searchData.length > 0 && (
              <div className={classes['filter-container']}>
                ...
                <div>
                  ...
                  <div className={classes['radio-but']}>
                    {[5, 4, 3, 2, 1].map((r) => {
                      return (
                        <div
                          onClick={() => starHandler(r)}
                          className={classes.star}
                          key={r}
                          id={r}>
                          <StarComponent value={r} />
                        </div>
                      );
                    })}
                  </div>
                ...
                <hr />
                <div>
                  <h5 className={classes['filter-label']}>
                    {' '}
                    Filter by Facility
                  </h5>
